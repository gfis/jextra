...
public class BaseParser {
    ...
    /** parser table with grammar and state set */
    protected Table table;
    /** current state of the push-down automaton */
    protected State state;
    /** Grammar of the language to be read in */
    protected Grammar grammar;
    /** scanner which reads the input file */
    protected Scanner scanner;
    /** category of the scanned symbol */
    protected int category;
    /** symbol just read by the scanner */
    ...
    protected boolean parse() { 
        if (Parm.isDebug(1)) {
            System.out.println(Parm.getXmlDeclaration() + Parm.getIndent() + "<Parser>");
            Parm.incrIndent();
        }
